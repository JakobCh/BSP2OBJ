...
def main():
    try:
        opts, args = getopt.getopt(sys.argv[1:], "o:p:m:c:e:")
        ...
        if (bspPath is None and extBspPath is None):
        ...
        if pakPath is not None and (bspPath is not None or extBspPath is not None):
            ...
            with open(path, "rb") as f:
                ...
                pak = PAK(stream)
                bsp = None
                if extBspPath is not None:
                    eF = open(extBspPath, "rb")
                    newStream = BinaryStream(eF)
                    bsp = BSP(stream, pak, palettePath, extBspStream=newStream)
                else:
